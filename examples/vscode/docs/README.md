...
```bash
./scripts/install.sh --host vscode --scope workspace --profile single
...
- a workspace package generated by `scripts/cells_agent.py`
...
python3 scripts/cells_agent.py install \
  ...
  --target /path/to/cells-project
```

For standalone distribution, render the plugin on demand:

```bash
python3 scripts/cells_agent.py render --host vscode --output ./dist/vscode --force
```

The generated plugin is under `dist/vscode/plugin/`.
