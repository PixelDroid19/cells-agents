...
- Do not invent paths/APIs/events. Verify file paths before editing.
- If required source evidence is missing, return `partial` or `blocked` instead of guessing.
...
For i18n or translation-related tasks, consult `skills/cells-i18n/` plus official docs before changing literals/runtime setup, and keep locale files under `demo/locales` for Cells contexts.
