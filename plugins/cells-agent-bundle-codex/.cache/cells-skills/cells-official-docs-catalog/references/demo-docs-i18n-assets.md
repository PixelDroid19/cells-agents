...
// Update the repo's actual locale source for the touched surface
```

### Locale file location — decide from repo evidence

For component demos, `demo/locales/locales.json` is commonly the runtime bundle. Some repos also keep feature/app locale source elsewhere, so do not flatten everything into one path.
...
For component packages, `demo/locales/locales.json` can differ from the repo's main locale source when dependencies also need translations.
