...
- `skills/cells-test-creator/` third for test design/creation/update conventions
...
9. **Docs vs wrapper naming** — if the repo uses a local wrapper such as `cells lit-component:test` while official docs describe `cells component:test`, report both instead of flattening them into one name.
...
- Keep Cells-native commands canonical and report the documented Cells equivalent when the repo uses a wrapper
