...
Interpretation rule:
- official docs define framework behavior and naming
- active repo evidence defines local wrappers and path conventions
- bundle heuristics are defaults only
...
|---|---|---|
| `this.t('key') \|\| ''` | The i18n runtime renders the key itself as fallback — it is NOT falsy. This hides missing translations. | Check the repo’s actual locale source for that surface. For component/demo work this is usually `demo/locales/locales.json`. Never use `\|\| ''`. |
...
5. Keep locale parity in the repo’s actual locale source for the touched surface; for component/demo work this is usually `demo/locales/locales.json`
