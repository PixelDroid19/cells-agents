...
### 11.1 Locale path is contextual
> Source: compare repo evidence with the official i18n and demo docs before changing locale files.

```
Component/demo runtime bundle → demo/locales/locales.json
Feature/app/test locale source → repo-local convention such as locales/locales.json
```

Never invent a new locale path without repo evidence.
...
| Invented or unsupported locale path for the touched surface | CRITICAL — wrong path | 11.1 |
